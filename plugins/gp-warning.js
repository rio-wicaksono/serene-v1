...
let handler = async (m, { conn, text, args, groupMetadata, usedPrefix, command }) => {      
        ...
        if (conn.user.jid.includes(who)) return m.reply(`✳️ Silakan sebutkan satu pengguna que no sea Bot`)
...
${mssg.wningUser(war)}`, who)
            ...
            m.reply(`⛔ ${mssg.warnMaxU}`)
            await time(3000)
            await conn.groupParticipantsUpdate(m.chat, [who], 'remove')
            m.reply(`♻️ Fuiste eliminado dari grup *${groupMetadata.subject}* karena telah diingatkan sebanyak *${war}* kali.`, who)
        }
}
