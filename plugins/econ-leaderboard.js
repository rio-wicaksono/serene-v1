...
let handler = async (m, { conn, args, participants }) => {
  ...
  let text = `_${mssg.resBt} 01/10/2023_
       ≡ *${mssg.lbTitle.toUpperCase()}*
    
▢ *${mssg.top.toUpperCase()} ${len} ${mssg.money.toUpperCase()}* 🪙
${mssg.you} : *${usersBank.indexOf(m.sender) + 1}* ${mssg.of} *${usersBank.length}*

${sortedBank.slice(0, len).map(({ jid, bank }, i) => `*${i + 1}.* ${participants.some(p => areJidsSameUser(jid, p.id)) ? `*${conn.getName(jid)}*` : `@${jid.split`@`[0]}`} ➭ _${bank.toLocaleString()}_ 🪙`).join`\n`}
